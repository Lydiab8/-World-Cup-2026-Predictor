...
** Application en ligne :** *(lien à compléter après déploiement — voir la section [Déploiement](#-déploiement--mettre-en-ligne-gratuitement) ci-dessous)*
...
Le plus simple pour obtenir un lien public à partager (LinkedIn, etc.) est **Streamlit Community Cloud**, gratuit et fait pour ce genre de projet :

1. Crée un dépôt GitHub **public** et pousse ce projet dedans (`git push`)
2. Va sur **[share.streamlit.io](https://share.streamlit.io)** et connecte-toi avec ton compte GitHub
3. Clique sur **"New app"**, choisis ton dépôt, la branche (`main`), et le fichier `app.py`
4. Clique sur **Deploy** — au bout d'1 à 2 minutes, tu obtiens une URL du type `https://ton-app.streamlit.app`

C'est cette URL que tu peux partager sur LinkedIn : n'importe qui peut l'ouvrir et tester les prédictions sans rien installer.

⚠️ Le dossier `models/` (modèle entraîné) doit être présent dans le dépôt pour que l'app démarre instantanément. Si tu préfères ne pas versionner les artefacts du modèle, tu peux à la place ajouter une étape d'entraînement automatique au démarrage (`train_model.py` exécuté une fois si `models/goal_model.pkl` est absent) — non inclus ici pour garder un démarrage rapide.
...
- Intégrer des données complémentaires : classement FIFA officiel, valeur marchande des effectifs, données de forme (xG réel)

## 👤 Auteur

**Lydia Bouaoudia** — Ingénieure IA / Agentic AI Engineering
[LinkedIn](#) · [GitHub](#)
